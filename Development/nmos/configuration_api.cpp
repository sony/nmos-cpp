...
namespace nmos
{
    ...
    inline web::http::experimental::listener::api_router make_unmounted_configuration_api(node_model& model, get_control_protocol_class_descriptor_handler get_control_protocol_class_descriptor, get_control_protocol_datatype_descriptor_handler get_control_protocol_datatype_descriptor, get_control_protocol_method_descriptor_handler get_control_protocol_method_descriptor, get_read_only_modification_allow_list_handler get_read_only_modification_allow_list, nmos::remove_device_model_object_handler remove_device_model_object, nmos::add_device_model_object_handler add_device_model_object, control_protocol_property_changed_handler property_changed, slog::base_gate& gate_)
    {
        ...
        configuration_api.support(U("/rolePaths/") + nmos::patterns::rolePath.pattern + U("/bulkProperties/?"), methods::PATCH, [&model, get_control_protocol_class_descriptor, get_read_only_modification_allow_list, remove_device_model_object, add_device_model_object, &gate_](http_request req, http_response res, const string_t&, const route_parameters& parameters)
            {
                const auto role_path = parameters.at(nmos::patterns::rolePath.name);
                const nmos::api_version version = nmos::parse_api_version(parameters.at(nmos::patterns::version.name));

                return details::extract_json(req, gate_).then([res, &model, role_path, get_control_protocol_class_descriptor, get_read_only_modification_allow_list, remove_device_model_object, add_device_model_object, version, &gate_](value body) mutable
                    {
                        auto lock = model.read_lock();
                        auto& resources = model.control_protocol_resources;
                        const auto& resource = nc::find_resource_by_role_path(resources, role_path);
                        if (resources.end() != resource)
                        {
                            ...
                            try
                            {
                                ...
                                method_result = validate_set_properties_by_path(resources, *resource, backup_data_set, recurse, restore_mode, get_control_protocol_class_descriptor, get_read_only_modification_allow_list, remove_device_model_object, add_device_model_object);
                ...
                return details::extract_json(req, gate_).then([res, &model, role_path, get_control_protocol_class_descriptor, get_read_only_modification_allow_list, remove_device_model_object, add_device_model_object, version, &gate_](value body) mutable
                    {
                        ...
                        if (resources.end() != resource)
                        {
                            ...
                            try
                            {
                                ...
                                method_result = set_properties_by_path(resources, *resource, backup_data_set, recurse, restore_mode, get_control_protocol_class_descriptor, get_read_only_modification_allow_list, remove_device_model_object, add_device_model_object);
