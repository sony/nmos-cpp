...
namespace nmos
{
        ...
        void build_role_paths(const resources& resources, const nmos::resource& resource, const utility::string_t& base_role_path, std::set<utility::string_t>& role_paths)
        {
            if (resource.data.has_field(nmos::fields::nc::members))
            {
                ...
                for (const auto& member : members)
                {
                    ...
                    if (nmos::nc::is_block(nmos::details::parse_nc_class_id(nmos::fields::nc::class_id(member))))
                    {
    ...
    inline web::http::experimental::listener::api_router make_unmounted_configuration_api(node_model& model, get_control_protocol_class_descriptor_handler get_control_protocol_class_descriptor, get_control_protocol_datatype_descriptor_handler get_control_protocol_datatype_descriptor, get_control_protocol_method_descriptor_handler get_control_protocol_method_descriptor, filter_property_value_holders_handler filter_property_value_holders, nmos::remove_device_model_object_handler remove_device_model_object, nmos::add_device_model_object_handler add_device_model_object, control_protocol_property_changed_handler property_changed, slog::base_gate& gate_)
    {
        ...
        configuration_api.support(U("/rolePaths/") + nmos::patterns::rolePath.pattern + U("/?"), methods::GET, [&model, &gate_](http_request req, http_response res, const string_t&, const route_parameters& parameters)
            {
                ...
                else
                {
                    ...
                    auto method_result = details::make_nc_method_result_error({ nmos::nc_method_status::bad_oid }, U("Not Found; ") + role_path);
        ...
        configuration_api.support(U("/rolePaths/") + nmos::patterns::rolePath.pattern + U("/properties/") + nmos::patterns::propertyId.pattern + U("/?"), methods::GET, [&model, get_control_protocol_class_descriptor, &gate_](http_request req, http_response res, const string_t&, const route_parameters& parameters)
            {
                ...
                if (resources.end() != resource)
                {
                    ...
                    {
                        // resource not found for the role path
                        auto method_result = details::make_nc_method_result_error({ nmos::nc_method_status::property_not_implemented }, U("Not Found; ") + property_id);
                        set_reply(res, status_codes::NotFound, method_result);
                    }
                    else
                    {
                ...
                return details::extract_json(req, gate).then([&model, req, res, parameters, get_control_protocol_class_descriptor, get_control_protocol_datatype_descriptor, property_changed, gate](value body) mutable
                    {
                        ...
                        if (resources.end() != resource)
                        {
                            ...
                            {
                                // property not found
                                auto method_result = details::make_nc_method_result_error({ nmos::nc_method_status::bad_oid }, U("Not Found; ") + property_id);
                                set_reply(res, status_codes::NotFound, method_result);
                            }
                            else
                            {
        ...
        configuration_api.support(U("/rolePaths/") + nmos::patterns::rolePath.pattern + U("/bulkProperties/?"), methods::PATCH, [&model, get_control_protocol_class_descriptor, filter_property_value_holders, remove_device_model_object, add_device_model_object, &gate_](http_request req, http_response res, const string_t&, const route_parameters& parameters)
            {
                const auto role_path = parameters.at(nmos::patterns::rolePath.name);
                const nmos::api_version version = nmos::parse_api_version(parameters.at(nmos::patterns::version.name));

                return details::extract_json(req, gate_).then([res, &model, role_path, get_control_protocol_class_descriptor, filter_property_value_holders, remove_device_model_object, add_device_model_object, version, &gate_](value body) mutable
                    {
                        auto lock = model.write_lock();
                        auto& resources = model.control_protocol_resources;
                        const auto& resource = nc::find_resource_by_role_path(resources, role_path);
                        if (resources.end() != resource)
                        {
                            ...
                            try
                            {
                                ...
                                method_result = validate_set_properties_by_path(resources, *resource, backup_data_set, recurse, restore_mode, get_control_protocol_class_descriptor, filter_property_value_holders, remove_device_model_object, add_device_model_object);
        ...
        configuration_api.support(U("/rolePaths/") + nmos::patterns::rolePath.pattern + U("/bulkProperties/?"), methods::PUT, [&model, get_control_protocol_class_descriptor, filter_property_value_holders, remove_device_model_object, add_device_model_object, &gate_](http_request req, http_response res, const string_t&, const route_parameters& parameters)
            {
                const auto role_path = parameters.at(nmos::patterns::rolePath.name);
                const nmos::api_version version = nmos::parse_api_version(parameters.at(nmos::patterns::version.name));
                ...
                return details::extract_json(req, gate_).then([res, &model, role_path, get_control_protocol_class_descriptor, filter_property_value_holders, remove_device_model_object, add_device_model_object, version, &gate_](value body) mutable
                    {
                        ...
                        if (resources.end() != resource)
                        {
                            ...
                            try
                            {
                                ...
                                method_result = set_properties_by_path(resources, *resource, backup_data_set, recurse, restore_mode, get_control_protocol_class_descriptor, filter_property_value_holders, remove_device_model_object, add_device_model_object);
