...
namespace nmos
{
    namespace details
    {
        void node_behaviour_thread(nmos::model& model, load_cacerts_handler load_cacerts, registration_handler registration_changed, mdns::service_advertiser& advertiser, mdns::service_discovery& discovery, slog::base_gate& gate);

        // registered operation
        void initial_registration(nmos::id& self_id, nmos::model& model, const nmos::id& grain_id, load_cacerts_handler load_cacerts, slog::base_gate& gate);
        void registered_operation(const nmos::id& self_id, nmos::model& model, const nmos::id& grain_id, load_cacerts_handler load_cacerts, registration_handler registration_changed, slog::base_gate& gate);
    ...
    void node_behaviour_thread(nmos::model& model, load_cacerts_handler load_cacerts, registration_handler registration_changed, slog::base_gate& gate_)
    {
        ...
        details::node_behaviour_thread(model, std::move(load_cacerts), std::move(registration_changed), advertiser, discovery, gate);
    }

    // uses the specified DNS-SD implementation
    // callbacks from this function are called with the model locked, and may read or write directly to the model
    void node_behaviour_thread(nmos::model& model, load_cacerts_handler load_cacerts, registration_handler registration_changed, mdns::service_advertiser& advertiser, mdns::service_discovery& discovery, slog::base_gate& gate_)
    {
        nmos::details::omanip_gate gate(gate_, nmos::stash_category(nmos::categories::node_behaviour));
    ...
    // uses the default DNS-SD implementation
    void node_behaviour_thread(nmos::model& model, load_cacerts_handler load_cacerts, slog::base_gate& gate)
    {
        node_behaviour_thread(model, load_cacerts, {}, gate);
    }

    // uses the specified DNS-SD implementation
    void node_behaviour_thread(nmos::model& model, load_cacerts_handler load_cacerts, mdns::service_advertiser& advertiser, mdns::service_discovery& discovery, slog::base_gate& gate)
    {
        node_behaviour_thread(model, load_cacerts, {}, advertiser, discovery, gate);
    }

    void details::node_behaviour_thread(nmos::model& model, load_cacerts_handler load_cacerts, registration_handler registration_changed, mdns::service_advertiser& advertiser, mdns::service_discovery& discovery, slog::base_gate& gate)
    {
        ...
        for (;;)
        {
            ...
            case initial_registration:
                ...
                details::initial_registration(self_id, model, grain_id, load_cacerts, gate);
            ...
            case registered_operation:
                ...
                details::registered_operation(self_id, model, grain_id, load_cacerts, registration_changed, gate);
    ...
    namespace details
    {
        ...
        web::http::client::http_client_config make_registration_client_config(const nmos::settings& settings, load_cacerts_handler load_cacerts)
        {
            auto config = nmos::make_http_client_config(settings, load_cacerts);
            config.set_timeout(std::chrono::seconds(nmos::fields::registration_request_max(settings)));
            return config;
        }

        web::http::client::http_client_config make_heartbeat_client_config(const nmos::settings& settings, load_cacerts_handler load_cacerts)
        {
            auto config = nmos::make_http_client_config(settings, load_cacerts);
        ...
        void initial_registration(nmos::id& self_id, nmos::model& model, const nmos::id& grain_id, load_cacerts_handler load_cacerts, slog::base_gate& gate)
        {
            ...
            for (;;)
            {
                ...
                if (!registration_client)
                {
                    ...
                    registration_client.reset(new web::http::client::http_client(base_uri, make_registration_client_config(model.settings, load_cacerts)));
                }
        ...
        void registered_operation(const nmos::id& self_id, nmos::model& model, const nmos::id& grain_id, load_cacerts_handler load_cacerts, registration_handler registration_changed, slog::base_gate& gate)
        {
            ...
            for (;;)
            {
                ...
                if (!registration_client)
                {
                    ...
                    registration_client.reset(new web::http::client::http_client(base_uri, make_registration_client_config(model.settings, load_cacerts)));
                    heartbeat_client.reset(new web::http::client::http_client(base_uri, make_heartbeat_client_config(model.settings, load_cacerts)));
