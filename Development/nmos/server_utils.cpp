...
namespace nmos
{
    namespace experimental
    {
        ...
        {
            const auto& port_map = nmos::experimental::fields::port_map(settings).as_array();
            const auto found = std::find_if(port_map.begin(), port_map.end(), [&](const web::json::value& m)
            {
                return client_port == m.at(U("client")).as_integer();
            });
            return port_map.end() != found ? found->at(U("server")).as_integer() : client_port;
        }
    }
}
