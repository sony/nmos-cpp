...
namespace nmos
{
    namespace details
    {
        void node_system_behaviour_thread(nmos::model& model, load_cacert_handler load_cacert, system_global_handler system_changed, mdns::service_discovery& discovery, slog::base_gate& gate);

        void node_system_behaviour(nmos::model& model, load_cacert_handler load_cacert, system_global_handler system_changed, slog::base_gate& gate);
    ...
    void node_system_behaviour_thread(nmos::model& model, load_cacert_handler load_cacert, system_global_handler system_changed, slog::base_gate& gate_)
    {
        ...
        details::node_system_behaviour_thread(model, std::move(load_cacert), std::move(system_changed), discovery, gate);
    }

    // uses the specified DNS-SD implementation
    // callbacks from this function are called with the model locked, and may read or write directly to the model
    void node_system_behaviour_thread(nmos::model& model, load_cacert_handler load_cacert, system_global_handler system_changed, mdns::service_discovery& discovery, slog::base_gate& gate_)
    {
        nmos::details::omanip_gate gate(gate_, nmos::stash_category(nmos::categories::node_system_behaviour));
    ...
    void details::node_system_behaviour_thread(nmos::model& model, load_cacert_handler load_cacert, system_global_handler system_changed, mdns::service_discovery& discovery, slog::base_gate& gate)
    {
        enum
        {
            initial_discovery,
        ...
        for (;;)
        {
            ...
            case node_system_behaviour:
                details::node_system_behaviour(model, load_cacert, system_changed, gate);
    ...
    namespace details
    {
        web::http::client::http_client_config make_system_client_config(const nmos::settings& settings, load_cacert_handler load_cacert)
        {
        ...
        void node_system_behaviour(nmos::model& model, load_cacert_handler load_cacert, system_global_handler system_changed, slog::base_gate& gate)
        {
            ...
            for (;;)
            {
                ...
                if (!state.client)
                {
                    ...
                    state.client.reset(new web::http::client::http_client(base_uri, make_system_client_config(model.settings, load_cacert)));
                }
