...
namespace nmos
{
    ...
    {
        return [&model, load_cacert, parse_transport_file, validate_merged](const nmos::id& receiver_id, const web::json::value& sender_data, slog::base_gate& gate)
        {
            ...
            if (sender_data.size() != 0)
            {
                ...
                web::http::client::http_client client(manifest_href, nmos::with_read_lock(model.mutex, [&model, load_cacert] { return nmos::make_http_client_config(model.settings, load_cacert); }));
    ...
    {
        return make_node_api_target_handler(model, load_cacert, &nmos::parse_rtp_transport_file, {});
    }
}
