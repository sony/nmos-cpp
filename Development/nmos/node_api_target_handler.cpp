...
namespace nmos
{
    ...
    {
        return [&model, load_cacerts, parse_transport_file, validate_merged](const nmos::id& receiver_id, const web::json::value& sender_data, slog::base_gate& gate)
        {
            ...
            if (sender_data.size() != 0)
            {
                ...
                web::http::client::http_client client(manifest_href, nmos::with_read_lock(model.mutex, [&model, load_cacerts] { return nmos::make_http_client_config(model.settings, load_cacerts); }));
