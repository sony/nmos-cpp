...
namespace nmos
{
    ...
    // this handler can be used to (un)subscribe IS-07 Events WebSocket receivers with the specified handlers, when they are activated
    nmos::connection_activation_handler make_connection_events_websocket_activation_handler(load_cacerts_handler load_cacerts, events_ws_message_handler message_handler, events_ws_close_handler close_handler, const nmos::settings& settings, slog::base_gate& gate);

    inline nmos::connection_activation_handler make_connection_events_websocket_activation_handler(load_cacerts_handler load_cacerts, events_ws_message_handler message_handler, const nmos::settings& settings, slog::base_gate& gate)
    {
        return make_connection_events_websocket_activation_handler(std::move(load_cacerts), std::move(message_handler), {}, settings, gate);
    }
