...
namespace nmos
{
    namespace experimental
    {
        namespace fields
        {
            ...
            // key_algorithm: (attribute of "server_certificates" object) type of key algorithm for the certificate, which can only be "ECDSA" or "RSA"
            const web::json::field_as_string_or key_algorithm{ U("key_algorithm"), U("") };

            // private_key_file: (attribute of "server_certificates" object) full path of private key file in PEM format
            const web::json::field_as_string_or private_key_file{ U("private_key_file"), U("") };

            // certificate_chain_file: (attribute of "server_certificates" object) full path of server certificate chain file in PEM format, which must be sorted
            // starting with the server's certificate, followed by any intermediate CA certificates, and ending with the highest level (root) CA
            const web::json::field_as_string_or certificate_chain_file{ U("certificate_chain_file"), U("") };

            // server_certificate_chains [registry, node]: an array of server certificate chain objects, each has the name of the key algorithm, the full paths of private key file and server certificate chain file
            // each value must be an object like { "key_algorithm": "ECDSA/RSA", "private_key_file": "server-key.pem, "certificate_chain_file": "server-chain.pem"}
            // see key_algorithm, private_key_file and certificate_chain_file above
            const web::json::field_as_value_or server_certificate_chains{ U("server_certificate_chains"), web::json::value::array() };
