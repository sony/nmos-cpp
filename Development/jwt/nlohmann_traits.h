#ifndef JWT_NLOHMANN_H
#define JWT_NLOHMANN_H
        ...
        namespace details
        {
            struct nlohmann_traits {
                ...
                static jwt::json::type get_type(const json &val)
                {
                    ...
                    else if (val.type() == json::value_t::number_integer) { return type::integer; }
                    // nlohmann internally tracks two types of integers
                    else if (val.type() == json::value_t::number_unsigned) { return type::integer; }
                    else if (val.type() == json::value_t::number_float) { return type::number; }
                    else if (val.type() == json::value_t::string) { return type::string; }
                    else if (val.type() == json::value_t::array) { return type::array; }
                    else if (val.type() == json::value_t::object) { return type::object; }
