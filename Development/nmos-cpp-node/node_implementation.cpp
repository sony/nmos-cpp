        ...
        // example sender-monitor(s)
        {
            ...
            for (int index = 0; index < how_many; ++index)
            {
                for (const auto& port : rtp_sender_ports)
                {
                    ...
                    const auto sender_monitor = nmos::make_sender_monitor(++oid, true, nmos::root_block_oid, role.str(), nmos::fields::label(sender->data), nmos::fields::description(sender->data), value_of({ { nmos::details::make_nc_touchpoint_nmos({nmos::ncp_touchpoint_resource_types::sender, sender_id}) } }));
