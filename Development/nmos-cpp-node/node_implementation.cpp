...
a=mid:SECONDARY
    ...
    const auto insert_resource_after = [&](unsigned int milliseconds, nmos::resource&& resource, slog::base_gate& gate)
    {
        if (!nmos::details::wait_for(model.shutdown_condition, lock, std::chrono::milliseconds(milliseconds), [&] { return model.shutdown; }))
        {
            ...
            if (nmos::types::sender == id_type.second) insert_resource(model.connection_resources, nmos::make_connection_sender(id_type.first, true, utility::s2us(smpte2022_7_video_sdp)));
            else if (nmos::types::receiver == id_type.second) insert_resource(model.connection_resources, nmos::make_connection_receiver(id_type.first, true));
    ...
    // example device
    insert_resource_after(delay_millis, nmos::make_device(device_id, node_id, { sender_id }, { receiver_id }, model.settings), gate);
    // example source
    insert_resource_after(delay_millis, nmos::make_video_source(source_id, device_id, { 25, 1 }, model.settings), gate);
    // example flow
    insert_resource_after(delay_millis, nmos::make_raw_video_flow(flow_id, source_id, device_id, model.settings), gate);
    // example sender
    {
