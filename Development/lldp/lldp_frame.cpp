    ...
    namespace details
    {
        ...
        size_t parse_tlv(const uint8_t* data, size_t len, tlv& tlv)
        {
            ...
            if (data && len >= min_tlv_size)
            {
                ...
                throw lldp_exception("TLV value field is shorter than expected");
            }
        ...
        chassis_id parse_chassis_id(const std::vector<uint8_t>& value)
        {
            ...
            {
                throw lldp_exception("missing Chassis ID subtype");
            }
        }
        ...
        port_id parse_port_id(const std::vector<uint8_t>& value)
        {
            ...
            {
                throw lldp_exception("missing Port ID subtype");
            }
        }
        ...
        std::chrono::seconds parse_time_to_live(const std::vector<uint8_t>& value)
        {
            ...
            throw lldp_exception("insufficient bytes for Time To Live");
        }
        ...
        system_capabilities parse_system_capabilities(const std::vector<uint8_t>& value)
        {
            ...
            {
                throw lldp_exception("invalid length for System Capabilities");
            }
        ...
        management_address parse_management_address(const std::vector<uint8_t>& value)
        {
            ...
            if (len < 1)
            {
                throw lldp_exception("missing management address string length for Management Address");
            }
            auto management_address_string_length = value.at(idx++);
            --len;

            // management address subtype
            if (len < 1)
            {
                throw lldp_exception("missing management address subtype for Management Address");
            }
            management_address.network_address.push_back(value.at(idx++));
            --len;
            --management_address_string_length;

            // management address
            switch (management_address.network_address.at(0))
            {
            case network_address_family_numbers::ipv4:
            {
                const size_t ipv4_size(4);
                if (management_address_string_length > len || ipv4_size != management_address_string_length)
                {
                    throw lldp_exception("invalid IPv4 address string for Management Address");
                }
                management_address.network_address.insert(management_address.network_address.end(), value.begin() + idx, value.begin() + idx + management_address_string_length);
            }
            break;
            case network_address_family_numbers::ipv6:
            {
                const size_t ipv6_size(8);
                if (management_address_string_length > len || ipv6_size != management_address_string_length)
                {
                    throw lldp_exception("invalid IPv6 address string for Management Address");
                }
                management_address.network_address.insert(management_address.network_address.end(), value.begin() + idx, value.begin() + idx + management_address_string_length);
            }
            break;
            ...
            {
                const size_t mac_size(6);
                if (management_address_string_length > len || mac_size != management_address_string_length)
                {
                    throw lldp_exception("invalid MAC address string for Management Address");
                }
                management_address.network_address.insert(management_address.network_address.end(), value.begin() + idx, value.begin() + idx + management_address_string_length);
            }
            break;
            case network_address_family_numbers::dns:
                ...
                {
                    throw lldp_exception("invalid DNS address string for Management Address");
                }
                management_address.network_address.insert(management_address.network_address.end(), value.begin() + idx, value.begin() + idx + management_address_string_length);
                break;
            case network_address_family_numbers::reserved:
            default:
                throw lldp_exception("invalid Management Address subtype");
            }
            if (parse_network_address(management_address.network_address).empty())
            {
                throw lldp_exception("invalid address string for Management Address");
            }
            idx += management_address_string_length;
            len -= management_address_string_length;

            // interface numbering subtype
            if (len < 1)
            {
                throw lldp_exception("missing interface numbering subtype for Management Address");
            }
            ...
            {
                throw lldp_exception("invalid Unknown interface numbering subtype");
            }

            // OID string length
            if (len < 1)
            {
                throw lldp_exception("missing OID string length for Management Address");
            }
            ...
            {
                throw lldp_exception("invalid OID string length for Management Address");
            }
        ...
        lldp_data_unit parse_lldp_data_uint(const uint8_t* data, size_t len)
        {
            ...
            for (auto it = ordered_mandatory_TLVs.begin(); it != ordered_mandatory_TLVs.end(); ++it)
            {
                ...
                if (*it != tlv.type)
                {
                    throw lldp_exception("the three mandatory TLVs are not in the expected order");
                }
            ...
            while (len > 0)
            {
                ...
                default:
                    // ignore
                    //std::cout << "ignore unsupported TLV type: " << (int)tlv.type << std::endl;
                    break;
                }
                data += consumed;
                len -= consumed;
        ...
        std::vector<uint8_t> make_lldp_frame(const lldp_frame& lldp_frame)
        {
            ...
            {
                throw lldp_format_error(e.what());
            }
        }
